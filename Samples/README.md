...
Each project imports `ServicePlugin.Packaging` so building them produces `.peakiot` archives under `bin/<configuration>/<tfm>/plugins`. Use these samples to validate packaging changes locally or in CI pipelines by adding `dotnet build Samples/TcpRelayPlugin` and `dotnet build Samples/HttpRelayPlugin` to the workflow.
