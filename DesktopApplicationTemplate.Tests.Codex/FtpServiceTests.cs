...
namespace DesktopApplicationTemplate.Tests
{
    public class FtpServiceTests
    {
        ...
        {
            var client = new Mock<IFtpClientWrapper>();
            client.Setup(c => c.Host).Returns("host");
            client.Setup(c => c.Port).Returns(21);
            client.Setup(c => c.Connect(It.IsAny<CancellationToken>())).Returns(Task.CompletedTask);
            client.Setup(c => c.UploadFile("local","remote", FtpRemoteExists.Overwrite, true, FtpVerify.None, null, It.IsAny<CancellationToken>())).Returns(Task.FromResult(FtpStatus.Success));
            ...
            client.Verify(c => c.UploadFile("local","remote", FtpRemoteExists.Overwrite, true, FtpVerify.None, null, It.IsAny<CancellationToken>()), Times.Once);
